...
const parseTransactions = (transactionFile: string): TransactionDTO[] => {
  const transactionsChunks = transactionFile.split('\n').filter(line => line.length > 0)
...
const createTransactionsRelations = async (transactions: TransactionDTO[]) => {
  const transactionsRelations = await Promise.all(transactions.sort((a, b) => a.type - b.type).map(async transaction => {
    const seller = await sellerService.findByNameOrCreate(transaction.sellerName)
    const product = await productService.findByNameOrCreate(transaction.productName, seller.id)
    ...
    return {
      ...
      sellerId: seller.id, 
      type: transactionTypeEnum[transaction.type as keyof typeof transactionTypeEnum]
    } 
  }))
